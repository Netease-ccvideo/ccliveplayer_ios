...
{
    memset(profiler, 0, sizeof(CCSDL_Profiler));
    if (max_sample < 0)
...
{
    profiler->begin_time = CCSDL_GetTickHR();
}

int64_t CCSDL_ProfilerEnd(CCSDL_Profiler* profiler)
{
    int64_t delta = CCSDL_GetTickHR() - profiler->begin_time;
...
{
    memset(sampler, 0, sizeof(CCSDL_SpeedSampler));
...
{
    Uint64 current = CCSDL_GetTickHR();
