...
IjkMediaPlayer *ijkmp_ios_create(int (*msg_loop)(void*),  int crop)
{
    ...
    mp->ffplayer->vout = CCSDL_VoutIos_CreateForGLES2();
    if (!mp->ffplayer->vout)
        goto fail;

//    mp->ffplayer->aout = CCSDL_AoutIos_CreateForAudioUnit();
    if (!mp->ffplayer->vout)
        goto fail;
...
void ijkmp_ios_set_glview_l(IjkMediaPlayer *mp, IJKSDLGLView *glView)
{
    ...
    CCSDL_VoutIos_SetGLView(mp->ffplayer->vout, glView);
}
...
void ijkmp_ios_set_frame_processor(IjkMediaPlayer *mp, FrameProcessor *fp)
{
    ...
    assert(mp->ffplayer->vout);
    pthread_mutex_lock(&mp->mutex);
    CCSDL_VoutIos_SetFrameProcessor(mp->ffplayer->vout, fp);
    pthread_mutex_unlock(&mp->mutex);
}

void jikmp_ios_set_volume(IjkMediaPlayer *mp, int volume)
{
    ijkmp_set_volume(mp, volume);
}
...
void ijkmp_ios_set_video_saver(IjkMediaPlayer *mp, const char *save_path){
    ijkmp_set_video_saver(mp, save_path);
}
