...
struct Ijk_VideoToolBox_Opaque {
    ...
    CCSDL_mutex                  *sample_info_mutex;
    CCSDL_cond                   *sample_info_cond;
...
{
    int total_wait = 0;
    CCSDL_LockMutex(context->sample_info_mutex);
    ...
    while (wait_ms < 0 || total_wait < wait_ms) {
        ...
        int wait_step = 10;
        CCSDL_CondWaitTimeout(context->sample_info_cond, context->sample_info_mutex, wait_step);
        total_wait += wait_step;
    }

    CCSDL_UnlockMutex(context->sample_info_mutex);
}
...
inline static sample_info* sample_info_peek(Ijk_VideoToolBox_Opaque* context)
{
    ...
    while (sample_info->is_decoding) {
        ...
        CCSDL_CondWaitTimeout(context->sample_info_cond, context->sample_info_mutex, 10);
    }
...
inline static void sample_info_recycle(Ijk_VideoToolBox_Opaque* context, sample_info *sample_info)
{
    ...
    CCSDL_CondSignal(context->sample_info_cond);
    CCSDL_UnlockMutex(context->sample_info_mutex);
}
...
//        ffp->stat.vdps = CCSDL_SpeedSamplerAdd(&ctx->sampler, FFP_SHOW_VDPS_VIDEOTOOLBOX, "vdps[VideoToolbox]");
...
void videotoolbox_async_free(Ijk_VideoToolBox_Opaque* context)
{
    ...
    if (context) {
        ResetPktBuffer(context);
        CCSDL_DestroyCondP(&context->sample_info_cond);
        CCSDL_DestroyMutexP(&context->sample_info_mutex);
    }
...
Ijk_VideoToolBox_Opaque* videotoolbox_async_create(FFPlayer* ffp, AVCodecContext* avctx)
{
    ...
    context_vtb->sample_info_mutex = CCSDL_CreateMutex();
    context_vtb->sample_info_cond  = CCSDL_CreateCond();
