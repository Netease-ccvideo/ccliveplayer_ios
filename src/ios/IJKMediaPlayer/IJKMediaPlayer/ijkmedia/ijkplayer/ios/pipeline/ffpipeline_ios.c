...
static IJKFF_Pipenode *func_open_video_decoder(IJKFF_Pipeline *pipeline, FFPlayer *ffp)
{
    ...
    if (ffp->videotoolbox) {
        ALOGD("use videotoolbox decode \n");
        node = ffpipenode_create_video_decoder_from_ios_videotoolbox(ffp);
    }
    if (node == NULL) {
        ALOGD("use ffmpeg decode \n");
...
static CCSDL_Aout *func_open_audio_output(IJKFF_Pipeline *pipeline, FFPlayer *ffp)
{
    return CCSDL_AoutIos_CreateForAudioUnit();
}

static CCSDL_Class g_pipeline_class = {
    .name = "ffpipeline_ios",
};
