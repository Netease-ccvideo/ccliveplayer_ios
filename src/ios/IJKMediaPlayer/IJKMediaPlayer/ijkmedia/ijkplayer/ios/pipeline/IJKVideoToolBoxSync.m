...
#define IJK_VTB_FCC_AVCC   CCSDL_FOURCC('C', 'c', 'v', 'a')

#define SESSION_REFRESH_MAX_COUNT 10
...
struct Ijk_VideoToolBox_Opaque {
    ...
    CCSDL_SpeedSampler            sampler;

    int                         serial;
    bool                        dealloced;
    
    volatile bool               decode_abort;
    volatile int                refresh_count;
...
#pragma mark -- 判断h264 profile硬解支持
...
#pragma mark -- from VLC github

static BOOL isSupportVTD(int profile, int level) {
    ...
    switch (profile) {
        ...
        default:
        {
            ALOGE("unsupported H264 profile %d \n", profile);
            return NO;
        }
    }
     ...
     * try to decode it*/
    if (level > 52) {
        ALOGE("unsupported H264 level %d \n", level);
        return NO;
    }
#else
    ...
    if (level > 42) {
        ...
        if (!deviceSupportsAdvancedLevels() || level > 52) {
            ALOGE("unsupported H264 level %d \n", level);
            return NO;
        }
    }
...
static VTDecompressionSessionRef vtbsession_create(Ijk_VideoToolBox_Opaque* context)
{
    ...
    VTSessionSetProperty(vt_session, kVTDecompressionPropertyKey_RealTime, kCFBooleanTrue);
    VTSessionSetProperty(vt_session, kVTDecompressionPropertyKey_ThreadCount, (__bridge CFTypeRef)@(4));
...
static int decode_video_internal(Ijk_VideoToolBox_Opaque* context, AVCodecContext *avctx, const AVPacket *avpkt, int* got_picture_ptr)
{
    ...
    if (status != 0) {
        ...
        if(status == kVTVideoDecoderBadDataErr) {
            //stop
            context->decode_abort = true;
        }
        if(context->refresh_count > SESSION_REFRESH_MAX_COUNT) { //失败次数太多
            ALOGF("context refresh_count max limit %d \n",context->refresh_count);
            context->decode_abort = true;
        }
        goto failed;
    } else {
        ...
        if (size_data && size_data_size > 7) {
            ...
            if (ret < 0) {
                ...
                if (context->codecpar->width  != new_avctx->width &&
                    ...
                    ALOGF("resolution changed refresh_request \n");
                }
            }
...
static void videotoolbox_write_mp4_descr_length2(PutByteContext *pb, int length)
{
    int i;
    uint8_t b;
...
static CFDataRef videotoolbox_esds_extradata_create2( const uint8_t *extradata, int extradata_size)
{
    CFDataRef data;
    uint8_t *rw_extradata;
    PutByteContext pb;
    int full_size = 3 + 5 + 13 + 5 + extradata_size + 3;
    // ES_DescrTag data + DecoderConfigDescrTag + data + DecSpecificInfoTag + size + SLConfigDescriptor
    int config_size = 13 + 5 + extradata_size;
    int s;

    if (!(rw_extradata = av_mallocz(full_size + 12)))
        return NULL;

    bytestream2_init_writer(&pb, rw_extradata, full_size + 12);
    ...
    videotoolbox_write_mp4_descr_length2(&pb, full_size);
    ...
    videotoolbox_write_mp4_descr_length2(&pb, config_size);
    ...
    videotoolbox_write_mp4_descr_length2(&pb, extradata_size);

    bytestream2_put_buffer(&pb, extradata, extradata_size);
...
static CMFormatDescriptionRef CreateFormatDescriptionFromCodecData(Uint32 format_id, int width, int height, const uint8_t *extradata, int extradata_size, uint32_t atom)
{
    ...
    switch (format_id) {
        ...
        {
            CFDataRef data = videotoolbox_esds_extradata_create2(extradata, extradata_size);
            if (data) {
...
int videotoolbox_sync_decode_frame(Ijk_VideoToolBox_Opaque* context)
{
    ...
    do {
        ...
        if(context->decode_abort) {
            return EIJK_VTB_FAILED;
        }
...
static int vtbformat_init(VTBFormatDesc *fmt_desc, AVCodecParameters *codecpar)
{
    ...
    switch (codec) {
        case AV_CODEC_ID_MPEG4:
            format_id = kCMVideoCodecType_MPEG4Video;
            fmt_desc->fmt_desc = CreateFormatDescriptionFromCodecData(kCMVideoCodecType_MPEG4Video, width, height, extradata, extrasize,  IJK_VTB_FCC_AVCC);
        ...
        case AV_CODEC_ID_HEVC:
            ...
            if (!isHevcSupported) {
                ALOGF("vtb current device is not support hevc (a9 class & ios 11)\n");
                goto fail;
            }
            break;
    ...
    if (extradata[0] == 1 && (format_id == kCMVideoCodecType_H264 || format_id == kCMVideoCodecType_HEVC)) {
        ...
        if(format_id == kCMVideoCodecType_HEVC) {
            if (!hevc_parse(extradata, extrasize, &fmt_desc->max_ref_frames)) {
                goto fail;
            }
        }
    ...
    } else if(format_id == kCMVideoCodecType_H264 || format_id == kCMVideoCodecType_HEVC){
        if ((extradata[0] == 0 && extradata[1] == 0 && extradata[2] == 0 && extradata[3] == 1) ||
            (extradata[0] == 0 && extradata[1] == 0 && extradata[2] == 1)) {
            AVIOContext *pb;
            ALOGI("%s - Extradata have startcode", __FUNCTION__);
            if (avio_open_dyn_buf(&pb) < 0) {
                goto fail;
            }
            ...
            if(format_id == kCMVideoCodecType_H264) {
                ...
                ALOGI("%s - h264 convert annex to avcc--->extradata size(%d)\n", __FUNCTION__, extrasize);
                extradata = NULL;
            ...
            // 如果是annex格式，将包含起始码的extradata转换为hvcc格式
            if(format_id == kCMVideoCodecType_HEVC) {
                ff_isom_write_hvcc(pb, extradata, extrasize, 0);
                ALOGI("%s - hevc convert annex to hvcc--->extradata size(%d)\n", __FUNCTION__, extrasize);
                extradata = NULL;
                extrasize = avio_close_dyn_buf(pb, &extradata);
                if (!hevc_parse(extradata, extrasize, &fmt_desc->max_ref_frames)) {
                    av_free(extradata);
    ...
    if(format_id == kCMVideoCodecType_H264) {
        if(profile == -99 || level == -99) {
            if(!isSupportVTD(sps_profile, sps_level)) {
                ALOGE(" profile or level no support for vtd \n");
                goto fail;
            }
        } else {
            if(!isSupportVTD(profile, level)) {
                ALOGE(" profile or level no support for vtd \n");
                goto fail;
            }
        }
...
Ijk_VideoToolBox_Opaque* videotoolbox_sync_create(FFPlayer* ffp, AVCodecContext* avctx)
{
    ...
    ffp->max_ref_frames = context_vtb->fmt_desc.max_ref_frames;
    ...
    CCSDL_SpeedSamplerReset(&context_vtb->sampler);
    return context_vtb;

fail:
    videotoolbox_sync_free(context_vtb);
    if(context_vtb) free(context_vtb);
    return NULL;
}
