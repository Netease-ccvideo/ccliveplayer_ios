...
typedef struct CCSDL_Aout_Opaque {
    IJKSDLAudioQueueController *aoutController;
} CCSDL_Aout_Opaque;

int aout_open_audio(CCSDL_Aout *aout, CCSDL_AudioSpec *desired, CCSDL_AudioSpec *obtained)
{
    assert(desired);
    SDLTRACE("aout_open_audio()\n");
    CCSDL_Aout_Opaque *opaque = aout->opaque;
...
void aout_pause_audio(CCSDL_Aout *aout, int pause_on)
{
...
void aout_flush_audio(CCSDL_Aout *aout)
{
...
void aout_close_audio(CCSDL_Aout *aout)
{
    ...
    @catch (NSException *exception) {
        SDLTRACE("aout_close_audio raise exception");
    }
}

static void aout_set_playback_rate(CCSDL_Aout *aout, float playbackRate)
{
    SDLTRACE("aout_set_playback_rate() %f\n",playbackRate);
    CCSDL_Aout_Opaque *opaque = aout->opaque;
    
    [opaque->aoutController setPlaybackRate:playbackRate];
}

static void aout_set_playback_volume(CCSDL_Aout *aout, float volume)
{
    SDLTRACE("aout_set_playback_volume() %f\n", volume);
    CCSDL_Aout_Opaque *opaque = aout->opaque;
    
    [opaque->aoutController setPlaybackVolume:volume];
}

static double aout_get_latency_seconds(CCSDL_Aout *aout)
{
    CCSDL_Aout_Opaque *opaque = aout->opaque;
    return [opaque->aoutController get_latency_seconds];
}

static int aout_get_persecond_callbacks(CCSDL_Aout *aout)
{
    return SDL_IOS_AUDIO_MAX_CALLBACKS_PER_SEC;
}

void aout_free_l(CCSDL_Aout *aout)
{
    if (!aout)
        return;
    ...
    CCSDL_Aout_FreeInternal(aout);
}

void aout_set_volume(CCSDL_Aout *aout, float left, float right)
{
    CCSDL_Aout_Opaque *opaque = aout->opaque;
    if (opaque) {
        [opaque->aoutController setVolume:left];
    }
}

CCSDL_Aout *CCSDL_AoutIos_CreateForAudioUnit()
{
    CCSDL_Aout *aout = CCSDL_Aout_CreateInternal(sizeof(CCSDL_Aout_Opaque));
    if (!aout)
        return NULL;
