...
- (id)initWithAudioSpec:(CCSDL_AudioSpec *)aSpec
{
    self = [super init];
    if (self) {
        if (aSpec == NULL) {
        ...
        CCSDL_CalculateAudioSpec(&_spec);
        ...
        for (int i = 0;i < kIJKAudioQueueNumberBuffers; i++)
        {
            ...
            memset(_audioQueueBufferRefArray[i]->mAudioData, 0, _audioQueueBufferRefArray[i]->mAudioDataByteSize);
...
{
    ALOGF("audioqueuectr setPlaybackRate %f \n",playbackRate);
...
static void IJKSDLAudioQueueOuptutCallback(void * inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer) {
    @autoreleasepool {
        ...
        if (!aqController) {
            // do nothing;
		} else if ([IJKMuteAudioManager sharedInstance].mute) {
