...
//add the lock, ensure that CREATE and RELEASE will not run at the same time.
static NSRecursiveLock *sCreateAndReleaseLock = NULL;
...
- (id) initWithFrame:(CGRect)frame sharegroup:(EAGLSharegroup*)sharegroup
{
    ...
    if (self) {
        if (!sCreateAndReleaseLock) {
            sCreateAndReleaseLock = [[NSRecursiveLock alloc] init];
        }
        _sharegroup = sharegroup;
        
        self.glActiveLock = [[NSRecursiveLock alloc] init];
        self.glActivePaused = NO;
        self.glActivePaused = ([UIApplication sharedApplication].applicationState != UIApplicationStateActive);
...
- (void)initContext
{
    ...
    NSLog(@"OK setup GL in thread %p", pthread_self());
    //    if ([EAGLContext currentContext] == _context)
    //        [EAGLContext setCurrentContext:nil];
    
    _videoCrop = NO;
    
    [sCreateAndReleaseLock unlock];
}
...
- (BOOL)setupDisplay: (CCSDL_VoutOverlay *) overlay
{
    ...
    if (_renderer == nil) {
        if (overlay == nil) {
            return NO;
        } else if (overlay->format == CCSDL_FCC__VTB) {
        ...
        } else if (overlay->format == CCSDL_FCC_I420) {
            _frameChroma = overlay->format;
            _renderer = [[IJKSDLGLRenderI420 alloc] init];
            _bytesPerPixel = 1;
            NSLog(@"OK use I420 GL renderer");
        } else if (overlay->format == CCSDL_FCC_RV24) {
            _frameChroma = overlay->format;
...
- (double)computeCropWidthRatio {
    double dH = _backingHeight * 1.0f / _frameHeight;
    double ratio = _backingWidth / (dH * _frameWidth);
...
- (void)updateVertices
{
    ...
    if (_videoCrop) {
        width *= [self computeCropWidthRatio];
    }
...
- (void)displayVout:(CCSDL_Vout*)vout overlay:(CCSDL_VoutOverlay *)overlay
{
...
//        NSLog(@"IJKSDLGLView:display: unable to tryLock GL active\n");
    ...
    [self displayInternalVout:vout overlay:overlay];
...
- (void)displayInternalVout:(CCSDL_Vout*)vout  overlay:(CCSDL_VoutOverlay *)overlay
{
    if (_context == nil) {
        [self initContext];
        if(_context == nil){
    ...
    EAGLContext* preCtx = [EAGLContext currentContext];
    [EAGLContext setCurrentContext:_context];
    
    if (![self setupDisplay:overlay]) {
        if ([EAGLContext currentContext] == _context)
            [EAGLContext setCurrentContext:nil];
        [EAGLContext setCurrentContext:preCtx];
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, _framebuffer);
    //bind framebuffer callback
    vout->on_bind_frame_buffer(vout, overlay);
    ...
    if ([_renderer prepareDisplay]) {
        ...
        double cropRatio = 0.5f;
        if (overlay->crop) {
            cropRatio = [self computeCropWidthRatio] / 2.0f;
        }
        ...
        if (_videoCrop) {
            if (_backingWidth > 0
                ...
                && _frameWidth > 0
                && _frameHeight > 0
                && (_backingWidth * _frameHeight != _backingHeight * _frameWidth)) {
                float backingAspect = _backingWidth / (float) _backingHeight;
                ...
                if (backingAspect > frameAspect) {
                    ...
                    _texCoords[0] = 0.0f;
                    _texCoords[1] = 1.0f - verticalCrop;
                    _texCoords[2] = 1.0f - _rightPadding;
                    _texCoords[3] = 1.0f - verticalCrop;
                    _texCoords[4] = 0.0f;
                    _texCoords[5] = verticalCrop;
                    _texCoords[6] = 1.0f - _rightPadding;
                    _texCoords[7] = verticalCrop;
                } else {
                    float horizontalCrop = (_frameWidth - backingAspect * _frameHeight) / 2.0f / (float)_pitchWidth;
                    _texCoords[0] = horizontalCrop;
                    _texCoords[1] = 1.0f;
                    _texCoords[2] = 1.0f - _rightPadding - horizontalCrop;
                    _texCoords[3] = 1.0f;
                    _texCoords[4] = horizontalCrop;
                    _texCoords[5] = 0.0f;
                    _texCoords[6] = 1.0f - _rightPadding - horizontalCrop;
                    _texCoords[7] = 0.0f;
                }
            } else {
                _texCoords[0] = 0.0f;
...
//                [EAGLContext setCurrentContext:nil];
	    ...
	    glBindRenderbuffer(GL_RENDERBUFFER, _renderbuffer);
        //before presentRenderbuffer callback
        vout->on_pre_render_frame(vout, overlay);
        [_context presentRenderbuffer:GL_RENDERBUFFER];
        //after presentRenderbuffer callback
        vout->on_post_render_frame(vout, overlay);
        

        int64_t current = (int64_t)CCSDL_GetTickHR();
