...
- (void) display: (CCSDL_VoutOverlay *) overlay
{
    assert(overlay->planes);
    assert(overlay->format == CCSDL_FCC__VTB);
    ...
    CVPixelBufferRef pixelBuffer = CCSDL_VoutOverlayVideoToolBox_GetCVPixelBufferRef(overlay);
    if (!pixelBuffer) {
